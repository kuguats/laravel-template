                            ...
                            <form class="layui-form">
                                @foreach($group->options as $option)
                                    <div class="layui-form-item">
                                        <label for="" class="layui-form-label" style="width: 120px">{{$option->label}}</label>
                                        <div class="layui-input-inline" style="min-width: 600px">
                                            @switch($option->type)
                                                @case('input')
                                                    <input type="input" class="layui-input" name="{{$option->key}}" value="{{$option->val}}">
                                                    @break
                                                @case('textarea')
                                                    <textarea name="{{$option->key}}" class="layui-textarea">{{$option->val}}</textarea>
                                                    @break
                                                @case('select')
                                                    <select name="{{$option->key}}">
                                                        @if($option->content)
                                                            @foreach(explode("|",$option->content) as $v)
                                                                ...
                                                                @endphp
                                                                <option value="{{$key}}" @if($key==$option->val) selected @endif >{{$val}}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                    @break
                                                    ...
                                                    @if($option->content)
                                                        @foreach(explode("|",$option->content) as $v)
                                                            @php
                                                                ...
                                                                $val = \Illuminate\Support\Str::after($v,':');
                                                            @endphp
                                                            <input type="radio" name="{{$option->key}}" value="{{$key}}" @if($key==$option->val) checked @endif title="{{$val}}">
                                                        @endforeach
                                                    @endif
                                                    @break
                                                @case('image')
                                                    <div class="layui-upload">
                                                        ...
                                                        <div class="layui-upload-list" >
                                                            <ul class="layui-upload-box layui-clear">
                                                                @if($option->val)
                                                                    <li><img src="{{ $option->val }}" /><p>上传成功</p></li>
                                                                @endif
                                                            </ul>
                                                            <input type="hidden" class="layui-upload-input" name="{{$option->key}}" value="{{$option->val}}">
                                                        </div>
                                                    </div>
                                                    @break
                                                @default
                                ...
                                @endforeach
                                    <div class="layui-form-item">
                                        <div class="layui-inline">
                                            <button type="submit" class="layui-btn" lay-submit lay-filter="option_group">确 认</button>
                                        </div>
                                    </div>
                            </form>
                        </div>
